...
# ---------------------------------------------------------------------------
# Shared bounded-resource helpers
# ---------------------------------------------------------------------------
...
def _validate_limits(max_depth: int, max_files: int) -> None:
...
def resource_budget(ram_available_mb: int, per_worker_mb: int, cpu_count: int | None = None, max_workers: int = 8) -> int:
    ram_workers = max(1, ram_available_mb // max(1, per_worker_mb))
...
def validate_loopback_host(host: str) -> str:
    """Validate the HTTP bind host; non-loopback exposure is never implicit."""
...
def subtitle_index(root: Path, *, max_depth: int = DEFAULT_MAX_DEPTH, max_files: int = DEFAULT_MAX_FILES) -> list[dict[str, Any]]:
    ...
    for p in _files(root, max_depth=max_depth, max_files=max_files):
        if p.suffix.lower() in SUBTITLE_EXTENSIONS:
            ...
            out.append({"path": str(p), "stem": p.stem, "language_hint": p.suffix.lower(), "size": size})
    return out
...
def content_index(paths: Iterable[Path], *, max_bytes: int = DEFAULT_SAMPLE_BYTES) -> dict[str, Any]:
    docs: dict[str, Any] = {}; df: dict[str, int] = {}
    for p in paths:
        text = text_extract(p, max_bytes=max_bytes)
        if not text: continue
        tokens = tokenize(text); counts = {t: tokens.count(t) for t in set(tokens)}
...
def ollama_generate(prompt: str, *, model: str = "qwen2.5-coder:7b", host: str = "127.0.0.1", port: int = 11434, timeout: int = 120) -> str:
    ...
    conn.request("POST", "/api/generate", body, {"Content-Type": "application/json"}); response = conn.getresponse(); data = json.loads(response.read().decode())
...
def create_snapshot(root: Path, destination: Path, parent: dict[str, Any] | None = None, *, max_depth: int = DEFAULT_MAX_DEPTH, max_files: int = DEFAULT_MAX_FILES) -> dict[str, Any]:
    files = manifest_tree(root, max_depth=max_depth, max_files=max_files); parent_files = (parent or {}).get("files", {}); changed = {k: v for k, v in files.items() if parent_files.get(k) != v}; deleted = sorted(set(parent_files) - set(files))
...
def restore_plan(snapshot: dict[str, Any], destination: Path) -> dict[str, Any]:
    ...
    for rel, meta in sorted(snapshot.get("files", {}).items()):
        target = destination / rel; action = "CREATE" if not target.exists() else "VERIFY_EXISTING"; items.append({"source": rel, "destination": str(target), "sha256": meta["sha256"], "action": action})
...
# ---------------------------------------------------------------------------
class CASStore:
    """Lab-owned content-addressable store; source files are never deleted."""
...
def compressed_snapshot(snapshot_path: Path, destination: Path, *, max_bytes: int = 100 * 1024 * 1024) -> dict[str, Any]:
    if snapshot_path.stat().st_size > max_bytes: raise ResourceLimitError(f"snapshot exceeds max_bytes={max_bytes}")
    destination.parent.mkdir(parents=True, exist_ok=True)
    with snapshot_path.open("rb") as src, gzip.open(destination, "wb") as dst:
        shutil.copyfileobj(src, dst, length=1024 * 1024)
    with gzip.open(destination, "rb") as check: verified = check.read(max_bytes + 1) == snapshot_path.read_bytes()
    return {"source": str(snapshot_path), "destination": str(destination), "bytes_before": snapshot_path.stat().st_size, "bytes_after": destination.stat().st_size, "verified": verified}
...
def ai_workflow_plan(steps: Iterable[dict[str, Any]]) -> dict[str, Any]:
    normalized = [{"id": s.get("id", f"step-{i + 1}"), "kind": s.get("kind", "advisory"), "requires_policy": True, "requires_authorization": True, "requires_verification": True, "index": i} for i, s in enumerate(steps)]
