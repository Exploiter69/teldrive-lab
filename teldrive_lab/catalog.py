...
class Catalog:
    ...
    def upsert(self, record: FileRecord) -> FileRecord:
        """Insert or update one observation without touching source content."""
        identity = (record.source_type.value, record.source_identifier, record.path)
        with self._connect() as connection:
            existing = connection.execute(
                """SELECT id, first_seen_at FROM files
                   WHERE source_type = ? AND source_identifier = ? AND path = ?""",
                identity,
            ).fetchone()
            first_seen = existing["first_seen_at"] if existing else record.first_seen_at
            connection.execute(
                """INSERT INTO files (
                    path, name, parent_path, size, mime_type, extension,
                    created_at, modified_at, sha256, hash_state,
                    source_type, source_identifier, destination_type,
                    destination_identifier, telegram_file_id, telegram_message_id,
                    telegram_channel_id, encryption_class, verification_state,
                    tags, job_id, first_seen_at, last_seen_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(source_type, source_identifier, path) DO UPDATE SET
                    ...
                    encryption_class = excluded.encryption_class,
                ...
                (
                    ...
                    record.destination_type.value if record.destination_type else None,
                    record.destination_identifier,
                    record.telegram_file_id,
                    record.telegram_message_id,
                    record.telegram_channel_id,
                    record.encryption_class.value,
    ...
    def get(self, source_type: SourceType, source_identifier: str, path: str) -> FileRecord | None:
        with self._connect() as connection:
            row = connection.execute(
                "SELECT * FROM files WHERE source_type = ? AND source_identifier = ? AND path = ?",
                (source_type.value, source_identifier, path),
            ).fetchone()
        ...
        with self._connect() as connection:
            rows = connection.execute(
                """SELECT * FROM files
                   ...
                   ORDER BY path""",
                (source_type.value, source_identifier),
            ).fetchall()
    ...
    def reconcile_source(
        ...
        """Report catalog records absent from an observation, without deleting them.

        Reconciliation is deliberately non-destructive. Callers can surface these
        records as stale/missing evidence and choose a separately authorized policy.
        """
        seen = set(seen_paths)
    ...
    def rebuild(self) -> None:
        """Reset only the Lab catalog; production data is never touched."""
        with self._connect() as connection:
    ...
    def close(self) -> None:
        """Compatibility no-op; connections are short-lived and transaction-scoped."""
        return None


def open_default_catalog() -> Catalog:
    """Create/open the default Lab catalog after ensuring Lab-owned runtime paths."""
