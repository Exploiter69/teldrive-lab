...
class BackupPlan:
    ...
    def digest(self) -> str:
        payload = "\n".join(
            f"{i.source}|{i.destination}|{i.size}|{i.sha256}" for i in self.items
...
class BackupStore:
    ...
    def __init__(self, path: str | Path | None = None) -> None:
        ...
        with sqlite3.connect(self.path) as db:
            ...
            if row is None:
                ...
                raise RuntimeError(f"unsupported backup schema {row[0]}")
    ...
    def lock(self, digest: str) -> None:
        with sqlite3.connect(self.path) as db:
            changed = db.execute("UPDATE backups SET locked=1 WHERE digest=?", (digest,)).rowcount
            if not changed:
...
class BackupPlanner:
    ...
    def plan(self, sources: list[str | Path] | tuple[str | Path, ...], *, backup_root: str | Path,
             policy: BackupPolicy | None = None, now: datetime | None = None) -> BackupPlan:
        ...
        for raw in sorted((Path(p) for p in sources), key=lambda p: str(p)):
            ...
            if not decision.allowed and not decision.requires_authorization:
                raise PermissionError(decision.reason or "protected boundary")
            if destination.exists():
                raise FileExistsError(destination)
            items.append(BackupItem(str(source), str(destination), source.stat().st_size, self._hash(source)))
        created = now.isoformat()
        purge_after = (now + timedelta(seconds=policy.effective_retention_seconds)).isoformat()
        manifest = str((root / "manifests" / f"{now.strftime('%Y%m%dT%H%M%SZ')}.json").resolve())
...
class BackupExecutor:
    """Executes only explicitly authorized Lab-owned backup/restore actions."""
    ...
    def _audit(operation: str, **kwargs: object) -> None:
        ...
        try:
            record_event(conn, event_id=str(uuid.uuid4()), operation=operation, **kwargs)
        finally:
            conn.close()
    ...
    def verify(self, plan: BackupPlan) -> tuple[BackupVerification, ...]:
        ...
        for item in plan.items:
            ...
            if not path.is_file():
                results.append(BackupVerification(item.destination, item.sha256, None, item.size, None, BackupState.MISSING))
                continue
    ...
    def restore(self, plan: BackupPlan, *, restore_root: str | Path, authorization_id: str) -> tuple[bool, tuple[str, ...]]:
        ...
        for item in plan.items:
            ...
            destination = (Path(restore_root).resolve() / Path(item.source).name).resolve()
            if destination.exists():
    ...
    def purge(self, plan: BackupPlan, *, authorization_id: str) -> tuple[bool, tuple[str, ...]]:
        ...
        removed: list[str] = []
        for item in plan.items:
            path = Path(item.destination).resolve()
        ...
        if manifest.exists():
            manifest.unlink()
        self._audit("BACKUP_PURGE", decision="ALLOW", result="SUCCESS", details={"digest": plan.digest, "authorization_id": authorization_id})
