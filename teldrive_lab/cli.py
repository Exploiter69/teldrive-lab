...
def _add_organization_parser(sub: argparse._SubParsersAction) -> None:
    ...
    parser.add_argument("--source-type", choices=[item.value for item in SourceType], required=True); parser.add_argument("--source-id", required=True)
    parser.add_argument("--raw-root", required=True); parser.add_argument("--crypt-root", required=True)
...
def _add_archive_parser(sub: argparse._SubParsersAction) -> None:
    ...
    parser.add_argument("--source-type", choices=[item.value for item in SourceType], required=True); parser.add_argument("--source-id", required=True); parser.add_argument("--archive-root", required=True)
    mode = parser.add_mutually_exclusive_group(); mode.add_argument("--dry-run", action="store_true"); mode.add_argument("--apply", action="store_true")


def _add_integrity_parser(sub: argparse._SubParsersAction) -> None:
    parser = sub.add_parser("verify", help="read-only SHA-256 verification report"); parser.add_argument("--source-type", choices=[item.value for item in SourceType], required=True); parser.add_argument("--source-id", required=True)


def _add_duplicate_parser(sub: argparse._SubParsersAction) -> None:
    parser = sub.add_parser("duplicates", help="read-only duplicate groups by size + SHA-256"); parser.add_argument("--source-type", choices=[item.value for item in SourceType], required=True); parser.add_argument("--source-id", required=True)


def _add_lifecycle_parser(sub: argparse._SubParsersAction) -> None:
    parser = sub.add_parser("lifecycle", help="plan or explicitly apply Lab-owned lifecycle actions"); parser.add_argument("action", choices=["purge", "restore"]); parser.add_argument("--root", required=True); parser.add_argument("--apply", action="store_true")
...
def _add_backup_parser(sub: argparse._SubParsersAction) -> None:
    ...
    parser.add_argument("action", choices=["plan", "apply", "verify", "restore"]); parser.add_argument("sources", nargs="+"); parser.add_argument("--root", required=True); parser.add_argument("--restore-root"); parser.add_argument("--apply", action="store_true")
...
def _verification_payload(report) -> dict:
    return {"total": len(report.results), "verified": report.verified, "missing": report.missing, "changed": report.changed, "mismatched": report.mismatched, "unverifiable": report.unverifiable,
            "items": [{"path": i.path, "expected_sha256": i.expected_sha256, "actual_sha256": i.actual_sha256, "expected_size": i.expected_size, "actual_size": i.actual_size, "state": i.state.value} for i in report.results]}
    ...
    try:
        ...
        if args.command == "backup":
            ...
            if args.action == "apply":
                ...
                success, changed = executor.apply(plan, authorization_id=f"cli:{uuid.uuid4()}"); print(json.dumps({"success": success, "changed": list(changed), "digest": plan.digest}, indent=2)); return 0 if success else 1
            if args.action == "verify":
                results = executor.verify(plan); print(json.dumps([asdict(r) for r in results], indent=2)); return 0 if all(r.state == "VERIFIED" for r in results) else 1
            if not args.restore_root or not args.apply: print("restore requires --restore-root and --apply"); return 2
            success, changed = executor.restore(plan, restore_root=args.restore_root, authorization_id=f"cli:{uuid.uuid4()}"); print(json.dumps({"success": success, "restored": list(changed), "digest": plan.digest}, indent=2)); return 0 if success else 1
        if args.command == "backup-schedule":
            scheduler = BackupScheduler()
            if args.action == "add":
                if not args.sources or not args.root: print("add requires sources and --root"); return 2
                print(json.dumps(asdict(scheduler.add(args.sources, args.root, interval_seconds=args.interval)), indent=2)); return 0
            print(json.dumps({"created_jobs": list(scheduler.run_due())}, indent=2)); return 0
    finally: audit.close()
    return 2
