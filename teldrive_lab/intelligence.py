...
def capability_report() -> dict[str, Any]:
    return {"tesseract": shutil.which("tesseract") is not None, "whisper": shutil.which("whisper") is not None, "ollama": shutil.which("ollama") is not None, "llama_cpp": shutil.which("llama-cli") is not None, "core_requires_ai": False, "authority": "advisory"}
...
def search_metadata(index: dict[str, Any], query: str, *, extension: str | None = None, min_size: int | None = None, max_size: int | None = None, limit: int = 20) -> list[dict[str, Any]]:
    q = _tokens(query)
    ...
    for record in index.get("records", []):
        ...
        query_name_tokens = [token for token in q if token != ext.lstrip(".")]
    ...
    return [{**by_path[x.path], "score": round(x.score, 6), "reasons": list(x.reasons)} for x in sorted(results, key=lambda x: (-x.score, x.path))[:limit]]

def structured_filter(records: Iterable[dict[str, Any]], *, extensions: Sequence[str] = (), min_size: int | None = None, max_size: int | None = None, name_pattern: str | None = None) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        if pattern and not pattern.search(name): continue
...
def parse_natural_language_query(query: str) -> dict[str, Any]:
    ...
    if "video" in text: ext = ext or "media-video"
    if "document" in text or "pdf" in text: ext = ext or ".pdf"
    size_min = None
...
def _digest_record(record: dict[str, Any]) -> tuple[str, int]:
    digest = str(record.get("sha256", "")); size = int(record.get("size", 0) or 0)
    return (digest, size) if digest else ("", size)
...
def duplicate_explanations(records: Iterable[dict[str, Any]]) -> list[IntelligenceProposal]:
    ...
    for record in records:
        ...
        if digest: groups.setdefault((digest, size), []).append(dict(record))
    out = []
...
def anomaly_explanations(records: Iterable[dict[str, Any]], *, large_file_bytes: int = 10 * 1024**3) -> list[IntelligenceProposal]:
    ...
    for record in records:
        path = str(record.get("path", record.get("name", "item"))); size = int(record.get("size", 0) or 0)
        if record.get("integrity_state") in {"FAILED", "UNVERIFIED"}: out.append(IntelligenceProposal("VERIFY", path, "Integrity evidence is missing or failed; verification should precede any lifecycle decision", .99))
        if size >= large_file_bytes: out.append(IntelligenceProposal("REVIEW_LARGE_OBJECT", path, f"Object is {size} bytes and deserves explicit transfer/cache planning", .85))
    return out

def organization_suggestions(records: Iterable[dict[str, Any]]) -> list[IntelligenceProposal]:
    out = []
    for record in records:
        path = str(record.get("path", record.get("name", "item"))); ext = str(record.get("extension", "")).casefold()
        if ext in MEDIA_EXTENSIONS: category = "media"
        elif ext in IMAGE_EXTENSIONS: category = "images"
        elif ext in {".pdf", ".docx", ".txt", ".md", ".rst"}: category = "documents"
        elif ext in {".srt", ".vtt", ".ass", ".ssa"}: category = "subtitles"
        else: continue
...
def metadata_enrichment_suggestions(records: Iterable[dict[str, Any]]) -> list[IntelligenceProposal]:
    out = []
    for record in records:
        path = str(record.get("path", record.get("name", "item"))); missing = [k for k in ("title", "mime") if not record.get(k)]
        if missing: out.append(IntelligenceProposal("ENRICH_METADATA", path, "Missing deterministic metadata: " + ", ".join(missing), .8))
    return out

def recommendation_report(records: Iterable[dict[str, Any]]) -> dict[str, Any]:
    records = list(records); suggestions = organization_suggestions(records) + metadata_enrichment_suggestions(records) + anomaly_explanations(records)
    return {"schema": SCHEMA, "recommendations": [asdict(x) for x in suggestions], "authority": "advisory", "mutation": "NONE"}

def ocr_index(path: Path, *, language: str = "eng", max_chars: int = 2_000_000, timeout: int = 60) -> dict[str, Any]:
    if shutil.which("tesseract") is None: return {"available": False, "path": str(path), "text": "", "reason": "tesseract_unavailable"}
...
def transcript_index(path: Path, *, model: str = "tiny", timeout: int = 120) -> dict[str, Any]:
    if shutil.which("whisper") is None: return {"available": False, "path": str(path), "text": "", "reason": "whisper_unavailable"}
    output_dir = path.parent / ".teldrive-lab-transcripts"; output_dir.mkdir(parents=True, exist_ok=True)
...
def summarize_text(text: str, *, max_sentences: int = 4) -> dict[str, Any]:
    sentences = [s.strip() for s in re.split(r"(?<=[.!?])\s+", text.strip()) if s.strip()]; selected = sentences[:max_sentences]
...
def local_model_adapter(prompt: str, *, model: str = "", host: str = "127.0.0.1", port: int = 11434, timeout: int = 30) -> dict[str, Any]:
    if not model: return {"available": False, "reason": "model_not_selected", "authority": "advisory"}
    try:
        import http.client
        body = json.dumps({"model": model, "prompt": prompt, "stream": False}); conn = http.client.HTTPConnection(host, port, timeout=timeout)
        conn.request("POST", "/api/generate", body, {"Content-Type": "application/json"}); response = conn.getresponse(); data = json.loads(response.read().decode("utf-8"))
        if response.status >= 400: return {"available": False, "reason": f"http_{response.status}", "authority": "advisory"}
        return {"available": True, "response": str(data.get("response", "")), "model": model, "authority": "advisory"}
    except (OSError, ValueError, json.JSONDecodeError): return {"available": False, "reason": "local_model_unavailable", "authority": "advisory"}

def export_intelligence(index: dict[str, Any], destination: Path) -> dict[str, Any]:
    payload = {"schema": SCHEMA, "records": index.get("records", [])}
    for record in payload["records"]: record.pop("_tokens", None)
    canonical = json.dumps(payload, sort_keys=True, separators=(",", ":")); payload["digest"] = hashlib.sha256(canonical.encode()).hexdigest()
...
def assert_advisory(proposals: Iterable[IntelligenceProposal]) -> None:
    for proposal in proposals:
        if proposal.authoritative or not proposal.requires_policy or not proposal.requires_authorization: raise ValueError("intelligence proposal cannot bypass policy/authorization")

__all__ = ["SCHEMA", "SearchResult", "IntelligenceProposal", "capability_report", "metadata_index", "search_metadata", "structured_filter", "parse_natural_language_query", "natural_language_search", "duplicate_explanations", "anomaly_explanations", "organization_suggestions", "metadata_enrichment_suggestions", "recommendation_report", "ocr_index", "transcript_index", "summarize_text", "ai_proposal", "local_model_adapter", "export_intelligence", "assert_advisory"]
